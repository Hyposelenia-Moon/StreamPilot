...
3. 若需以后一键打开，点 **＋新增预设**：在对话框中填 **主播名称**、**直播链接** 与 **平台**。
   粘贴链接后平台会按域名自动选中（抖音分享短链 `v.douyin.com`、B站短链 `b23.tv`、
   虎牙 / 斗鱼 / YY / Bigo 链接都能识别），识别不准或只填房间号（例如虎牙 `660000`）时可手动改选。
   确定时先按选定的平台解析该链接，成功才保存并在列表中选中；失败时在对话框内说明原因且不保存。
   点击列表里的预设即按该预设自己的平台解析该房间，不会一律当成哔哩哔哩。
...
> **虎牙 20M 档**：关于「蓝光20M」档位，平台要求通过手机虎牙 App 扫码登录后才授予 7 天观看权限；未完成扫码登录时平台不下发该档。本程序只如实列出平台返回的档位，不代做登录、不绕过权限；扫码需在虎牙官方 App 或网页自行完成，再将该会话的虎牙 Cookie 填入。
...
| C# 单元测试 | **146 / 146 通过**（`tests/StreamPilot.Tests/Cases/*.cs`） |
| 前端回归测试 | **51 / 51 通过**（`tests/web/player-core.test.js`，`node --test`） |
| 静态红线自检 | 通过 |
| `node build/analyze-csharp.mjs` | **105 文件 / 24086 行 / 161 类型 / 782 方法**，未发现结构性问题 |
...
> 上表的 C# 单元测试数量是**预设平台改动之前**的整轮结果：本轮新增 `PlatformDetectionTests`
> 与 `PresetStoreTests`（共 16 个用例），用例总数与 `build\test.ps1` 结论需在集成时按实际运行结果更新；
> `node build/analyze-csharp.mjs` 一行已按改动后的实际输出更新。
