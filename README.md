...
| 界面验证 | 主按钮与选中档位为白字（`TextBlock` 隐式样式不再覆盖按钮前景色）；「＋ 新增预设」始终可点并在下拉中选中新预设；播放中点击 150/250 ms 立即热切换（日志「已应用到当前播放」）且顶部提示显示真实档位 |
...
- **抖音签名**：不实现 `a_bogus` / `ms_token` / `__ac_signature` 等风控签名（属绕过平台风控）。因此：主页内嵌数据可用时正常解析；需要签名的备用接口会被平台以 `status_code=10011` 拒绝，此时提示「抖音备用接口拒绝了本次请求（该接口要求平台签名，本程序不实现）」而不是谎报「未开播」。
