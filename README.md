...
|----------|-------------------|
| 用电脑看直播，画面比网页更跟手 | 内置播放器，支持 150 / 200 / 250 ms 三档追帧，抖动时自动回落到稳定档 |
...
| 画面一顿一顿 / 反复重连 | 先看画面下方的「追帧档位」：150 ms 对网络最敏感，抖动时程序会自动回落到稳定档并提示；仍不稳就手动选 250 ms。程序已不做候选探测（探测会与播放争用同一条直播长连接） |
...
| 播放策略前端测试 | **30 / 30 通过**（`node --test --experimental-test-isolation=none tests/web/player-core.test.js`） |
...
| 本轮未能在本机复验的部分 | 播放页脚本、XAML、C# 均**未在本机运行**（沙箱禁止 `dotnet build`/`dotnet run`，且本机无法访问平台域名做真实解析）。卡顿修复、画质下拉与暂停/继续的端到端表现需在集成环境用真实房间复验（见 `docs/runbooks/troubleshooting.md`） |
