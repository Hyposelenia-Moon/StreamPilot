...
**用户实测**：`https://www.huya.com/152792` 这类房间的「蓝光20M」档，平台要求
**用手机虎牙 App 扫码登录**之后才授予 **7 天**观看权限；未完成扫码登录时平台**根本不下发该档位**。

因此：

- 本程序只把平台**如实返回**的档位列出来。平台不下发 20M 档，下拉里就不会有 `蓝光20M`——这不是解析器的 bug；
- StreamPilot **不代做登录、不绕权限、不做任何扫码/风控绕过**。扫码登录请在虎牙官方 App 或官网自行完成；
- 想在程序里用上该档位，请按 `README.md` 第 5 节把**你自己账号**的虎牙 Cookie 填进设置（解析请求会带上它），
  并且该 Cookie 必须来自已经完成扫码登录的会话；
- 权限到期（7 天）后平台会重新收回该档位，需要重新扫码登录并更新 Cookie；
- 档位是否出现只以接口响应为准：`data.liveData.bitRateInfo` / `rateArray` 里没有的档位，程序不会凭经验伪造。
