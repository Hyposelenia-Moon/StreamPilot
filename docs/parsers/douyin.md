...
- 输入支持 live.douyin.com/<房间号>，也支持分享链接的「首页 + 参数」形式：路径没有房间号时依次读查询参数 live_web_rid / web_rid / oom_id / oomid（只接受纯数字）。
- 备用 reflow 接口要求平台签名；被拒时平台返回 status_code=10011，本实现按 **Rejected**（平台拒绝）归类并说明原因，不再笼统报「解析错误」。
- 主页内嵌 JSON 抽取失败时只记 Debug 并继续走备用路径，不把「抽不出来」当成「未开播」。
