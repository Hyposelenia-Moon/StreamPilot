...
| 平台拒绝 | reflow 根级 `status_code` 非 0（数值或数字字符串都识别） | `抖音备用接口拒绝了本次请求：status_code=10011，message=Request params error。该接口按公开参数调用，不做平台签名。` |
...
- 页面提示浏览器安全验证（含 `__ac_nonce`）时停止解析，只给出可读提示，不做任何绕过尝试。
