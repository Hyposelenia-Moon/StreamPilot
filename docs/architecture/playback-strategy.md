...
- 侧栏档位（150/200/250 ms）在**播放中**点击时，宿主下发 `target` 消息而不是等下次播放
  （`docs/architecture/player-message-contract.md` 第 1.2.1 节）；页面改写当前运行对象的
  `extremeTargetMs/extremeTargetSeconds` 并立即追帧，日志出现「已应用到当前播放」；
- 没有活动会话时只保存配置，日志为「下次播放生效」；
