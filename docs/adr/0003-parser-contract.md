...
| 抖音 | `live.douyin.com/{roomId}` 页面 + 正则抽 `roomStore` JSON；失败回退 `webcast.amemv.com/webcast/room/reflow/info/?type_id=0&live_id=1&room_id={roomId}` | 画质 `FULL_HD1 → HD1 → SD1 → SD2`；`anchor == null` → `RoomNotFound`；`room == null` 或 `status == 4` → `NotLive`；**不实现** `a_bogus`/`ms_token`（参考项目自身未启用，且属于签名伪造，违反"不绕过风控"） |
