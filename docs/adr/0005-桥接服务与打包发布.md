...
dotnet publish src/StreamPilot.App/StreamPilot.App.csproj `
  ...
  -o "D:\文件\实用软件\b站插件\StreamPilot_publish"
```
...
- 产物结构（`publish.ps1` 负责组装，全部写入 `StreamPilot_publish`，**源码目录不落任何产物**）：

```
StreamPilot_publish\
  StreamPilot.exe                 # 单文件：.NET 运行时 + 托管程序集 + WebView2Loader
  Web\                            # 播放页与库（必须为磁盘文件：虚拟主机映射需要真实目录）
    player.html  mpegts.js  hls.js  decode-selftest.mp4  *-LICENSE.txt
...
- 构建脚本行为：
  ...
  2. `dotnet build -c Release` 编译全部工程并运行测试工程，测试非零退出码即中止发布；
  3. `dotnet publish` 到 `StreamPilot_publish`；
  4. 复制 `Web/`、`docs/`、`LICENSE`、`THIRD-PARTY-NOTICES.md`，生成 `VERSION.txt`（含 `git describe` 或时间戳）；
  5. 计算产物 SHA256 并写入 `VERSION.txt`，用于发布记录。
